...
@app.route('/utilizatori/<user_mail>/visits/<oras>/<marker_name>', methods=["POST"])
@cross_origin()
def visits(user_mail,oras, marker_name):
    ...
    if request.method == "POST":
        ...
        cursor.execute("Insert INTO Vizited Select Utilizator.user_id,Markers.marker_id FROM Markers "
                       "INNER JOIN Utilizator WHERE Markers.Oras='%s' and Markers.Titlu='%s' and Utilizator.mail='%s'"% (oras,marker_name,user_mail))
        out = cursor.fetchall()
        cnxn.commit()
        cnxn.close()
        myResponse = {
...
@app.route('/utilizatori/<user_mail>/visits/<oras>', methods=["GET"])
@cross_origin()
def visited(user_mail,oras):
    if request.method not in ["GET"]:
    ...
    if request.method == "GET":
        cursor.execute("Select Markers.marker_id, Markers.Titlu, Markers.Oras, Markers.Latitudine, Markers.Longitudine, Markers.URL  from Markers "
                       ...
                       "JOIN Utilizator ON Utilizator.user_id=Vizited.user_id and Utilizator.mail='%s' "
                       "WHERE Markers.Oras='%s'" % (user_mail,oras))
        out = cursor.fetchall()
        cnxn.close()
        myResponse = {
